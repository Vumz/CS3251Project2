...
while True:
    ...
    if 'tweet' == inputSplit[0]:
        ...
        if len(inputSplit) != 2:
            print('incorrect amount of arguments for unsubscribe command')
            client.sendall(bytes('none', 'UTF-8'))
        elif '#' in inputSplit[1]:
        ...
        if len(inputSplit) != 2:
            print('incorrect amount of arguments for subscribe command')
            client.sendall(bytes('none', 'UTF-8'))
        elif '#' in inputSplit[1]:
            hashtag = inputSplit[1]
        ...
        client_message = 'timeline ' + USERNAME
        client.sendall(bytes(client_message, 'UTF-8'))
